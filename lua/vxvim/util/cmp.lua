---@class vxvim.util.cmp
local M = {}

---@alias vxvim.util.cmp.Action fun():boolean?
---@type table<string, vxvim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.expand(snippet)
  ...
  -- See: https://github.com/VxUtil/VxUtil/issues/3199
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return VxUtil.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      VxUtil.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    VxUtil.cmp.add_missing_snippet_docs(event.window)
  end)
end
