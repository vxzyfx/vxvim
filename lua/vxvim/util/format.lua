...
function M.formatexpr()
  if VxVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  VxVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    VxVim.warn("No formatter available", { title = "VxVim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "vxvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    VxVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in VxVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `vxvim.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return VxVim.format.enabled()
    end,
    set = function(state)
      VxVim.format.enable(state, buf)
    end,
  })
end
